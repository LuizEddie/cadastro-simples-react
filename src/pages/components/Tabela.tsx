...
export default function Tabela(props: TabelaProps){
    ...
    function renderizarDados(){
        return props.clientes?.map((cliente, i)=>{
            ...
            const colorClass = i % 2 === 0 ? 'bg-purple-200' : 'bg-purple-100';

            return (
                <tr key={cliente.id} className={colorClass}>
    ...
    function renderizarAcoes(cliente: Cliente){
        return (
            <td className="flex justify-center">
                ...
                <button onClick={()=>props.clienteExcluido?.(cliente)} className={`flex justify-center items-center text-red-600 rounded-full hover:bg-purple-50 p-2 m-1`}>{IconeLixo}</button>
                ) : false}
            </td>
        );
    }
